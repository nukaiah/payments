...
class _HomeScreenState extends State<HomeScreen> {
  ...
  opeChe(){
    var  options = {
      ...
      'amount': num.parse(_payamountController.text)*100+(num.parse(_payamountController.text)/100)*200,
      'name': 'SnaggerIndia',
  ...
  Widget build(BuildContext context) {
    ...
    return isLoad? Scaffold(
      body:Stack(
        children: [
          AppBar(
            ...
            title: Column(
              children: [
                Text("Hey !",style: TxtStls.titlestl1,),
                username==null?Text("...",style: TxtStls.titlestl1):Text(username.toString(),style: TxtStls.titlestl1),
              ],
            ),
            ...
            actions: [
              ...
              }, icon: Icon(Icons.payment_outlined,color: bgClr)):SizedBox()
            ],
          ),
          Positioned(
            ...
            child: Card(
              ...
              child: SizedBox(
                ...
                child: Column(
                  ...
                  children: [
                    Text("Current Balance :",style: TxtStls.txtStl,),
                    cbal==null? Text("₹ 0",style: TxtStls.txtStl,):Text("₹ ${cbal}",style: TxtStls.txtStl,)
                  ],
                ),
              ),
            ),
            ...
            child:Card(
              ...
              child: Container(
                  padding: EdgeInsets.only(
                      left:size.width*0.1, right:size.width*0.1, top:size.height*0.002),
          ...
          textLength <= 0?SizedBox():Positioned(
            ...
            child: MaterialButton(
              ...
              child: Text("Tap To Pay",style: TxtStls.txtStl2,),
              onPressed: (){
                //opeChe();
      ...
      floatingActionButton:cbal==null||cbal==0?SizedBox():FloatingActionButton(
        elevation: 0,
    ...
    appBar:  AppBar(
      ...
      elevation: 0.0,
      title:
          Text("Payment",style: TxtStls.titlestl1,),
      ...
      body: SingleChildScrollView(
        child: Form(
          ...
          child: Column(
            children: [
              field(_uidController, "Enter id", (value){}),
  ...
  Future fetchUserData()async{
    ...
    PostgrestResponse postgrestResponse = await supabase.from("Users").select().eq("uuid", uuid).single().execute();
  ...
  Future addData()async{
    SharedPreferences preferences = await SharedPreferences.getInstance();
    var uuid = preferences.getString("uuid");
    PostgrestResponse postgrestResponse = await supabase.from("Transactions").insert({
  ...
  Future addData1()async{
    ...
    if(postgrestResponse.error  == null){
      ...
      final snackBar = SnackBar(backgroundColor:Colors.green,content: Text("Paid Successfully",style: TxtStls.txtStl2),shape: RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(10.0))),);
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
      setState(() {
      ...
      final snackBar = SnackBar(backgroundColor:Colors.green,content: Text("Failed! Try Again",style: TxtStls.txtStl2),shape: RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(10.0))),);
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
    }
  }
  ...
  Future fetchCurrealace()async{
    SharedPreferences preferences = await SharedPreferences.getInstance();
    var uuid = preferences.getString("uuid");
    PostgrestResponse postgrestResponse = await supabase.from("Transactions").select().eq("uuid", uuid).execute();
    var d = postgrestResponse.data;
      setState(() {
        cbal  = d.map((m) =>(m["trans"])).reduce((a,b )=>a+b);
      });
  }
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          content: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              Lottie.asset("assets/raiserequest.json",reverse: true,height: 100),
    ...
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          ...
          title:
          Text('Pay', style: TxtStls.titlestl),
          actions: <Widget>[
            Row(
